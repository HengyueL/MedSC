...
def impulse_noise(x, severity=1):
    c = [.03, .06, .09, 0.17, 0.27][severity - 1]  # the origina imagenet-c
    # c = [.03, 0.3, 0.5, 0.7, 0.27][severity - 1]  # the origina imagenet-c
...
def fog(x, severity=1):
    ...
    x += c[0] * plasma_fractal(mapsize=512,wibbledecay=c[1])[:512, :512][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    frost = resize(frost, (512, 512, 3), preserve_range=True).astype('uint8')
...
def corrupt_image(img, corruption_type, severity):
    if corruption_type == "none":
        ...
        corrupted_img = jpeg_compression(img, severity)
    elif corruption_type == "defocus_blur":
        corrupted_img = defocus_blur(img, severity)
